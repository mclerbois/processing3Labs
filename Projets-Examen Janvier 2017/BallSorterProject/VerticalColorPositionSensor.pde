...
public class VerticalColorPositionSensor extends Actor implements Runnable {
  private Actor target;
  private Indicator led;
  private int y1,y2;
  
  public VerticalColorPositionSensor(String[] imageFileNames, int x, int y, int y1, int y2) {
  ...
  void run() {
    for (;; ) {
        ...
        if(state)
          value=target.value; // 0 noir 1 blanc
    try { 
